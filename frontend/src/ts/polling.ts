...
class PollingController {
    ...
    videoHandler = (id: string) => {
        ...
        if (navigator.userActivation?.hasBeenActive && localStorage) {
            const kioskVideoIsMuted =
                storageUtils.get<boolean>("kioskVideoIsMuted");
        ...
        this.playTimeout = setTimeout(() => {
            if (
                this.video &&
                (this.video.paused || this.video.currentTime === 0)
            ) {
                console.error(
                    "Video failed to start playing within timeout period",
                );
